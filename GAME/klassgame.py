...
while not stop_flag:
    ...
    try:
        ...
        for button in buttons:
            ...
            if href == "https://www.klasgame.com/satis-yap/revenger-online/revenger-online-gold/revenger-online-1m":
        ...
        if correct_button:
            ...
            if onclick == "message('Şu an için alış aktif görünmüyor, lütfen daha sonra tekrar deneyiniz.', 'danger'); return false;":
                ...
                for uid in USER_IDS:
                    send_telegram_message(uid, f"Revenger Online satış aktif: {CHECK_URL}")
        else:
            log("⚠️ 10-GB butonu bulunamadı!")

        # --- Rastgele gezinme (eski mantık) ---
